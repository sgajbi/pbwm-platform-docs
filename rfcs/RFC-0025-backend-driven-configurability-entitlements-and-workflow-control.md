...
Feature switches, workflow variants, and entitlement logic can drift into UI/BFF if not explicitly centralized, causing inconsistent behavior across clients and environments.
...
- BFF performs contract shaping and pass-through of policy context.
...
2. `GET /entitlements` family at BFF boundary for user/session-specific controls.
...
2. BFF must not become policy engine.
3. Policy packs and feature toggles must be versioned and auditable.

## 6. Acceptance Criteria

1. PAS/PA/DPM each expose capability metadata endpoint.
2. BFF integration tests validate disabled-feature behavior consistently.
