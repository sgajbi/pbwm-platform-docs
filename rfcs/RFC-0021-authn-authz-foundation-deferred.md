...
- Current priority is workflow delivery speed and UI/BFF/DPM functional integration.
...
- JWT validation at BFF and service layer.
...
- Role + scope policy enforcement at BFF entry points.
- Resource/action policy checks in DPM domain endpoints.
- Explicit deny-by-default behavior for protected endpoints.

### 3. Cross-Service Propagation

- Pass authenticated user context from UI -> BFF -> DPM.
...
1. Proposal workflow slice is stable in UI + BFF + DPM.
...
- Protected endpoints enforce policies in BFF and DPM.
